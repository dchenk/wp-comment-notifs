...
if(!class_exists('WP_Comment_Notifs')) {
	class WP_Comment_Notifs{
		...
		function __construct(){
			...
			add_action('comment_post', array($this, 'init_notifications'), 999, 2);
			add_action('admin_menu', array($this, 'admin_menu'));
			add_action('admin_init', array($this, 'wp_comment_notification_settings_init'));
		}

		function admin_menu(){
			add_options_page(__('Comment Notifs', 'wp_comment_notifs'), __('Comment Notifs', 'wp_comment_notifs'), 'manage_options', 'wp-comment-notifs',
				[$this, 'settings_page']);
		}
		...
		function wp_comment_notification_settings_section_callback(){
			echo __('WP Comment Notification Setting','wp_comment_notifs');
		}
		...
		function get_notification_mail_ids(){
			$options = get_option('wp_comment_notification_settings');
			$wpcn_mails = isset($options['wp_comment_notification_emails']) ? $options['wp_comment_notification_emails'] :
		...
		function init_notifications($comment_id, $commentdata){
